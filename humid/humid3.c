...
void Initialize_U(float U[], int N) {
    // Define a small epsilon to correct for float division inaccuracies.
    // Increased the epsilon value from 1e-6 to 1e-5 to guarantee the value is nudged past the boundary.
    ...
    // Add EPSILON to ensure correct rounding up when the result is precisely on an integer boundary (e.g., 39.999... becomes 40)
    const int I_START = (int)(0.02f / DX + EPSILON); 
    const int I_END = (int)(0.03f / DX + EPSILON);
    ...
    // For DX = 0.0005: I_START should be 40, I_END should be 60. Pulse is in cells 40 through 59.
    for (int i = I_START; i < I_END; i++) {
        if (i < N) {
            U[i] = 1.0f;
        }
    }
    // Print the physical boundaries of the pulse (Left Boundary of I_START to Right Boundary of I_END-1)
